...
**状态**：方案草稿 (Draft)  
**适用对象**：课程研发团队、编译架构师、提示词工程师、教研助理  
**适用范围**：NCU 人机协同系列课程课件母本 (`.master.md`)、课堂演示幻灯片 (`.slides.md`)、出版教材源及自动化编译流水线  

---

## 1. 现状痛点与重构动因 (Problem Statement & Background)

在《人机协同程序设计》课程第 9 讲至第 14 讲的持续演进中，课件团队遭遇了显著的排版效能瓶颈与视觉质量分化。特别是第 11 讲耗费数日人工微调才勉强及格，而第 12 讲则在反复自动化打补丁后发生了严重的“排版压扁、留白尽失、字号忽大忽小、部分页面截断”的系统性劣质问题。

通过对各讲母本源码的系统解构，总结出传统排版模式下的四大核心病灶：
...
1. **认知负荷超载：让“内容智能体”兼顾“排版”的根本性设计缺陷**：
   过去的流水线要求负责撰写教学内容的智能体同时生成复杂的 Marp 排版代码。这直接导致了大模型注意力分散——既想把教学概念阐述透彻，又要在脑海中推演像素坐标，最终产出了一堆文字极度过载、又臭又长的“论文式幻灯片”。
2. **信息密度击穿 720px 垂直视口空间**：
   幻灯片不同于纸质教材，其有效可视高度被严格固化在 720px 以内。扣除上下留白（约 120px）与标题区（约 80px），留给正文卡片的垂直预算仅约 **500px**。第 9/10 讲单页字数克制在 120~180 字，因而自然舒展；而第 12 讲单页信息量激增至 350~500 字，在固定高度视口下必然发生截断溢出。
3. **开环无反馈（Open-Loop）下的盲目削足适履**：
   传统流程缺乏“渲染-测量-反馈-调整”的闭环反馈回路。智能体在盲人摸象的状态下随意猜测内联字号（`0.72em`、`0.75em`、`0.83em`）并注入 `<style scoped>` 强行压缩边距。最终导致全讲字号断层，页面被极度压扁。
4. **内联 HTML 泥潭摧毁单一事实源（SSOT）**：
   母本中大量侵入裸写 HTML（如 `<div class="columns ratio-4-6" style="...">`），导致 `.master.md` 丧失了纯净 Markdown 语义。当需要同时派生学生自学讲义（A4 版）或导出 Typst 印刷教材时，这些专有内联样式造成了严重的跨媒介样式污染。

---

## 2. 核心架构重构：三大工程范式深度融合

为了终结“反复修改、越修越丑、最后死锁”的工程困局，本系统将平台成熟的 **Harness 工程** 与 **Loop 工程** 全面引入幻灯片排版流水线，并与 **HTML 响应式设计（CDRD）** 深度整合：
...
* **解耦职责**：内容创作智能体（Node 05）只需输出纯粹的教学文本与大纲结构，**严禁考虑排版**；
* **专属后处理**：由独立的**后处理排版智能体（Node 09 Layout Agent）**接管初稿；
* **梯度自愈循环**：后处理智能体与无头沙盒构成闭环控制回路，在 $N \le 3$ 轮循环内依次执行“语义组件包装 $\to$ 响应式断点调度 $\to$ 结构化强制分页”，杜绝无限修改的震荡死锁。

---

## 3. 后处理排版智能体（Slide Layout Agent）系统架构

后处理排版智能体作为流水线 Node 09 的核心执行器，承担着将“原始粗糙初稿”转换为“高审美规范母本”的全部职责：
...
为了彻底杜绝第 11 讲“反复折腾好几天”的人工消耗与第 12 讲“越改越劣质”的死锁，后处理智能体遵循严格的**有限状态自愈收敛算法（Max Loops = 3）**：
...
```markdown
你是由 NCU 研发的“Marp 幻灯片后处理排版架构师智能体 (Node 09 Layout Agent)”。
你的唯一职责是：接收上游内容智能体编写的“未考虑排版问题的粗糙 Markdown 幻灯片”，在严格遵守 Harness 工程围栏与 Loop 自愈规则的前提下，将其重构为视觉层级清晰、自适应留白优雅的专业级幻灯片。

【绝对禁令 (Harness Redlines)】
1. 严禁输出任何形式的内联 CSS (禁止 style="..."，违者打回)；
2. 严禁注入 <style scoped> (边距与字号完全交由主题断点管理)；
3. 严禁主观删减、压缩或篡改原作者的教学论述、实验代码与提示词内容；
...
### 7.2 真实重构用例对照 (Transformation Case: Slide 07)

#### 输入：未考虑排版的原始粗糙初稿 (Raw Draft)
```markdown
## 核心框架：IPO 结构化规范建模法

![width:360px](image/第十二讲_规范_结构化表达与Spec提示词工程.master/ipo_framework_diagram.png)
...
#### 输出：经后处理智能体加工的规范成品 (Refactored Master)
```markdown
---
<!-- _class: ds-lg -->
...
![width:340px](image/第十二讲_规范_结构化表达与Spec提示词工程.master/ipo_framework_diagram.png)
...
1. **零内联样式率 (Zero-Inline-Style Rate = 100%)**：母本中 `style="font-size:"` 与 `<style scoped>` 绝对归零。
2. **零视口截断率 (Zero-Overflow Rate = 100%)**：无头 Chromium 批量核验，全讲 20~30 页幻灯片 `isOverflowing === false` 达成率 100%。
